...
class ReadMock(BaseCrudMock):
    ...
    def __init__(self):
        super().__init__()
        self.default_response = MockResponse(status_code=200, json_data={"id": 1, "name": "Read Resource"})
        ...
        def field_selection_response(**request_kwargs):
            ...
            if "fields" in params:
                ...
                if hasattr(original_response, "_json_data") and original_response._json_data:
                    ...
                    filtered_data = {field: original_response._json_data.get(field) for field in fields if field in original_response._json_data}
                    return MockResponse(status_code=200, json_data=filtered_data)

            # If no field selection or invalid fields, return None to use the default response
            return None

        self.with_response(url_pattern=url_pattern, response=field_selection_response, **kwargs)
        return self
    ...
    def with_filtering(self, url_pattern: str, **kwargs: Any) -> "ReadMock":
        def filtering_response(**request_kwargs):
            ...
            filter_params = {k: v for k, v in params.items() if k not in ("sort", "fields", "page", "limit")}
            ...
            if filter_params and self._stored_resources:
                ...
                return MockResponse(status_code=200, text=json.dumps(filtered_resources if len(filtered_resources) > 0 else []))

            # If no filtering or no stored resources, return None to use the default response
            return None

        self.with_response(url_pattern=url_pattern, response=filtering_response, **kwargs)
        return self
    ...
    def with_sorting(self, url_pattern: str, **kwargs: Any) -> "ReadMock":
        def sorting_response(**request_kwargs):
            ...
            if "sort" in params and self._stored_resources:
                ...
                return MockResponse(status_code=200, text=json.dumps(sorted_resources))

            # If no sorting or no stored resources, return None to use the default response
            return None

        self.with_response(url_pattern=url_pattern, response=sorting_response, **kwargs)
        return self
    ...
    def with_pagination(self, url_pattern: str, **kwargs: Any) -> "ReadMock":
        ...
        self.with_response(url_pattern=url_pattern, response=pagination_response, **kwargs)
