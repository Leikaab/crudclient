...
def create_mock_client(**kwargs):
    ...
    if "auth_type" in kwargs and "auth_config" in kwargs:
        ...
        if auth_type == "basic":
            basic_auth_mock = create_basic_auth_mock(username=auth_config.get("username", ""), password=auth_config.get("password", ""))
            mock_client.set_auth_strategy(basic_auth_mock.get_auth_strategy())
        elif auth_type == "bearer":
            bearer_auth_mock = create_bearer_auth_mock(token=auth_config.get("token", ""))
            ...
            api_key_auth_mock = create_api_key_auth_mock(
                api_key=auth_config.get("api_key", ""), header_name=auth_config.get("header_name"), param_name=auth_config.get("param_name")
            )
